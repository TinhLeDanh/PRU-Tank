public class GameConst
{
    public const string Tank_FileName = "Tank";
    public const string Tank_MenuName = "GameData/Entity/Tank";
    public const int Tank_Order = -9999;

    public const string ConstrucionTank_FileName = "Construction Tank";
    public const string ConstructionTank_MenuName = "GameData/Construction/Construction Tank";
    public const int ConstructionTank_Order = -9998;
    
    public const string ConstrucionStuff_FileName = "Construction Stuff";
    public const string ConstructionStuff_MenuName = "GameData/Construction/Construction Stuff";
    public const int ConstructionStuff_Order = -9997;

    public const string ConstrucionStuffList_FileName = "Construction Stuff List";
    public const string ConstructionStuffList_MenuName = "GameData/Construction/Construction Stuff List";
    public const int ConstructionStuffList_Order = -9996;
    
    public const string Item_FileName = "Item";
    public const string Item_MenuName = "GameData/Entity/Item";
    public const int Item_Order = -9995;
    
    public const string GameData_FileName = "GameData";
    public const string GameData_MenuName = "GameData/GameGeneralData";
    public const int GameData_Order = -9994;
}